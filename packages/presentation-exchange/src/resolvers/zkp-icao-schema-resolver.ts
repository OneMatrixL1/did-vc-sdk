...
export function createZKPICAOSchemaResolver(
  ...
  return {
    ...
    async deriveCredentialWithZKP(
      ...
      // Generate ZKP proofs for each condition
      ...
      // Build Merkle disclosure proofs for DG13 fields
      ...
      // Handle non-Merkle disclosed fields via base ICAO resolver
      if (nonMerkleConds.length > 0) {
        const nonMerkleDg13Fields = nonMerkleConds.filter((d) => isDg13Field(d.field));
        if (merkleConds.length > 0 && nonMerkleDg13Fields.length > 0) {
          throw new Error(
...
async function buildZKPProof(
  ...
  const result = await provider.prove({
    ...
    privateInputs: { merkleWitness: witness },
    publicInputs: {
