...
export async function verifyZKPProofs(
  ...
  // Check that all required conditions were satisfied
...
async function verifyZKPProofSingle(
  ...
  return { conditionID: proof.conditionID, type: 'ZKPProof', verified: true };
}
...
function verifyMerkleProofSingle(
  ...
  if (!verifyMerkleInclusion(proof, poseidon2)) {
    return {
      conditionID: proof.conditionID,
      type: 'MerkleDisclosureProof',
      verified: false,
  ...
  return { conditionID: proof.conditionID, type: 'MerkleDisclosureProof', verified: true };
}
...
function verifyMerkleInclusionInner(
  ...
  return reconstructedCommitment === BigInt(proof.commitment);
}
