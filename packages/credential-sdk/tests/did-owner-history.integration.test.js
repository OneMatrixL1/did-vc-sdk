...
const networkConfig = {
  ...
  registry: process.env.ETHR_REGISTRY_ADDRESS || DEFAULT_REGISTRY_ADDRESS, // registry default
};
...
describe("DIDServiceClient Owner History Verification", () => {
  ...
  beforeAll(async () => {
    ...
    if (process.env.ETHR_PRIVATE_KEY) {
      ...
      initialBBSKeypair = Bls12381BBSKeyPairDock2023.generate({
        id: "initial-owner",
      });
  ...
  test("generates valid signatures for each Dual DID transition (Mock Fallback)", async () => {
    ...
    for (const [i, record] of history.entries()) {
      ...
      const identity40Bytes = concat([
        parsed.secp256k1Address,
        parsed.bbsAddress,
      ]);
      expect(record.message.identity.toLowerCase()).toBe(
        identity40Bytes.toLowerCase(),
      );
  ...
  test("initializes with provider and verifies 3 on-chain transitions", async () => {
    if (!networkConfig.rpcUrl || !process.env.ETHR_PRIVATE_KEY) {
      console.warn(
        "Skipping on-chain test: ETHR_NETWORK_RPC_URL or ETHR_PRIVATE_KEY not set",
      );
      return;
    }
    ...
    for (let i = 1; i <= 3; i++) {
      const nextBBSKeypair = Bls12381BBSKeyPairDock2023.generate({
        id: `owner-${i}`,
      });
      ...
      const uncompressedPubkey = getUncompressedG2PublicKey(
        currentBBSKeypair.publicKeyBuffer,
      );
    ...
    const history = await client.getDIDOwnerHistory(did);
    console.log(
      `Fetched on-chain history for ${did}:`,
    ...
    for (let i = 0; i < 3; i++) {
      ...
      expect(record.message.oldOwner.toLowerCase()).toBe(
        expected.oldOwner.toLowerCase(),
      );
      expect(record.message.newOwner.toLowerCase()).toBe(
        expected.newOwner.toLowerCase(),
      );
      expect(record.publicKey.toLowerCase()).toBe(
        expected.publicKey.toLowerCase(),
      );
      ...
      if (i > 0) {
        ...
        // eslint-disable-next-line jest/no-conditional-expect
        expect(record.message.oldOwner.toLowerCase()).toBe(
          prevRecord.message.newOwner.toLowerCase(),
        );
      }
  ...
  test("initializes with provider and verifies Dual DID on-chain transitions", async () => {
    if (!networkConfig.rpcUrl || !process.env.ETHR_PRIVATE_KEY) {
      console.warn(
        "Skipping on-chain Dual DID test: ETHR_NETWORK_RPC_URL or ETHR_PRIVATE_KEY not set",
      );
      return;
    }
    ...
    // 1. Create a Dual DID
    const secondaryBBSKeypair = Bls12381BBSKeyPairDock2023.generate({
      id: "dual-bbs-owner",
    });
    const dualDID = await module.createDualAddressDID(
      gasPayerKeypair,
      secondaryBBSKeypair,
    );
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      console.log(
        `Dual DID Transition ${i}: changing owner to ${nextOwnerAddress}`,
      );
    ...
    const history = await client.getDIDOwnerHistory(dualDID);
    console.log(
      `Fetched on-chain history for ${dualDID}:`,
      JSON.stringify(history, null, 2),
    );
